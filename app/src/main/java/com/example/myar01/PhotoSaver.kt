...
class PhotoSaver (
    ...
    private fun generateFilename(): String?{
        ...
        return Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DCIM)?.absolutePath +
                "/TryOutFuniture/${date}_screenshot.jpg"
    }

    private fun saveBitmapToGallery(bmp: Bitmap){
        val date = SimpleDateFormat("yyyyMMddHHss", Locale.getDefault()).format(Date())
...
//            put(MediaStore.MediaColumns.RELATIVE_PATH, "DCIM/TryOutFurniture")
        ...
        val url = activity.contentResolver.insert(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, contentValues)

        activity.contentResolver.openOutputStream(url ?: return).use { outputStream->
            outputStream?.let {
                try {
        ...
        PixelCopy.request(arSceneView, bmp, { result ->
            if (result == PixelCopy.SUCCESS){
                ...
                activity.runOnUiThread {
                    Toast.makeText(activity, "Sucessfully took photo!",Toast.LENGTH_LONG).show()
                }
            }else {
                activity.runOnUiThread {
